...
void ShowNativeToast(std::string message) {
    std::thread([message]() {
        ...
        jobject looper =
            env->CallStaticObjectMethod(looperClass, myLooper);
        ...
        if (activityThread != nullptr) {
            ...
            if (context != nullptr) {
                ...
                if (toast != nullptr) {
                    jmethodID show =
                        env->GetMethodID(toastClass, "show", "()V");
        ...
        jmethodID loop =
            env->GetStaticMethodID(looperClass, "loop", "()V");
...
static uintptr_t GetVehicleFromIndex(uintptr_t game, uint16_t vehicleId) {
    ...
    if (vehicleId >= vehicleCount || vehicleId > 512) {
        return 0;
    }
...
static bool SendAllBytes(int socketFd, const void* data, size_t size) {
    if (socketFd < 0 || data == nullptr || size == 0) {
        return false;
    }
...
static void QueueVehiclePosition(
    ...
    if (ownerId == 0xFF || ownerId >= MAX_PLAYERS) {
        return;
    }
...
static void CaptureAndQueueLocalVehicleState(uintptr_t game) {
    ...
    uintptr_t vehicle =
        GetActiveVehicleFromGame(game, &vehicleId);
    ...
    // KEPT: never echo a vehicle owned by the other player
...
static void HandleVehiclePositionPacket(
    ...
    if (pkt.vehicleId >= MAX_VEHICLE_SLOTS) return;

    // KEPT: assign vehicle ownership from the sender
    {
        std::lock_guard<std::mutex> ownerLock(
            g_VehicleOwnerMutex
        );

        if (g_VehicleOwner[pkt.vehicleId] == 0xFF) {
            g_VehicleOwner[pkt.vehicleId] = pkt.ownerId;
        ...
        if (g_VehicleOwner[pkt.vehicleId] != pkt.ownerId) {
            LOGI(
                "[VEHICLE REJECTED] vehicle=%u owner=%u expected=%u",
                (unsigned)pkt.vehicleId,
                (unsigned)pkt.ownerId,
                (unsigned)g_VehicleOwner[pkt.vehicleId]
            );
    ...
    if (state.valid && pkt.sequence <= state.sequence) {
        return;
    }
    ...
    for (; it != end; ++it) {
        if (it->ifr_addr.sa_family == AF_INET) {
            struct sockaddr_in* addr =
                (struct sockaddr_in*)&it->ifr_addr;
...
void OpenAndroidKeyboard() {
    ...
    jint res = g_GlobalJavaVM->GetEnv(
        (void**)&env,
        JNI_VERSION_1_6
    );
    ...
    while (
        ...
        if (n > 0) {
            ...
            if (msg.find("FS14_PONG|") == 0) {
                ...
                for (auto& peer : g_DiscoveredPeers) {
                    if (peer.ip == devIP) {
                        exists = true;
                        break;
                    }
...
void TCPHostThread() {
    ...
    if (g_ConnectedStatus != "Room Closed.") {
        g_ConnectedStatus =
            "Connection Lost.";
    }
}
    ...
    while (true) {
        ...
        if (
            ...
            *outEvent != nullptr
            ...
            int32_t eventType =
                AInputEvent_getType(*outEvent);

            if (eventType == AINPUT_EVENT_TYPE_MOTION) {
                int32_t action =
                    AMotionEvent_getAction(*outEvent) &
                ...
                size_t pointerCount =
                    AMotionEvent_getPointerCount(*outEvent);
                ...
                eventType == AINPUT_EVENT_TYPE_KEY
            ) {
                int32_t action =
                    AKeyEvent_getAction(*outEvent);

                int32_t keyCode =
                    AKeyEvent_getKeyCode(*outEvent);

                int32_t metaState =
                    AKeyEvent_getMetaState(*outEvent);
...
void DrawImGui() {
    ...
    if (g_ImGuiInitialized) {
        ...
        if (g_IsMultiplayerMenuActive) {
            float winWidth =
                (float)viewport[2] * 0.82f;

            float winHeight =
                (float)viewport[3] * 0.88f;
...
void* my_updateGUI(
    ...
    return orig_updateGUI ?
        orig_updateGUI(thiz, p1, p2, p3, p4) :
        nullptr;
}
