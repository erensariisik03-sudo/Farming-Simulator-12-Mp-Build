...
typedef void* (*renderStartMenuMain_t)(
    void* thiz,
    void* p1,
    void* p2,
    void* p3
...
typedef int32_t (*AInputQueue_getEvent_t)(
    void* queue,
...
typedef void (*GameUpdateStateBase_t)(
    void* thiz,
    float param_1,
    uint32_t param_2,
    uint32_t param_3,
...
typedef void (*VehicleGetPosition_t)(
    void* vehicle,
    float* p1,
    float* p2
);

typedef float (*VehicleGetOrientation_t)(
    void* vehicle
);

typedef void (*b2BodySetTransform_t)(
    void* body,
    const TestB2Vec2* position,
    float angle
);
...
static_assert(
    sizeof(SessionWelcomePacket) == 4,
    "SessionWelcomePacket size mismatch"
);

static_assert(
    sizeof(NetworkPacket) == 256,
    "NetworkPacket size mismatch"
);

static_assert(
    sizeof(VehiclePositionPacket) == 20,
    "VehiclePositionPacket size mismatch"
);
...
static const uint16_t MAX_VEHICLE_SLOTS = 513;

// FF = henüz sahibi yok
static uint8_t g_VehicleOwner[MAX_VEHICLE_SLOTS];
static std::mutex g_VehicleOwnerMutex;

static void ResetVehicleOwners() {
    std::lock_guard<std::mutex> lock(g_VehicleOwnerMutex);

    for (uint16_t i = 0; i < MAX_VEHICLE_SLOTS; ++i) {
        g_VehicleOwner[i] = 0xFF;
    }
}

// Araç henüz sahipsizse bu cihaz alır.
// Zaten bu cihazın sahibiyse true.
// Başka oyuncunun sahibiyse false.
static bool TryClaimVehicle(uint16_t vehicleId) {

    if (vehicleId >= MAX_VEHICLE_SLOTS) {
        return false;
    }

    const uint8_t localId = g_LocalPlayerId.load();

    if (localId == 0xFF || localId >= MAX_PLAYERS) {
        return false;
    }

    std::lock_guard<std::mutex> lock(g_VehicleOwnerMutex);

    if (g_VehicleOwner[vehicleId] == 0xFF) {

        g_VehicleOwner[vehicleId] = localId;

        LOGI(
            "[VEHICLE OWNER] vehicle=%u claimed by player=%u",
            (unsigned)vehicleId,
            (unsigned)localId
        );
    ...
    return g_VehicleOwner[vehicleId] == localId;
}

// ========================================================================
// VEHICLE POINTER HELPERS
// ========================================================================

static uintptr_t GetVehicleFromIndex(
    uintptr_t game,
    uint16_t vehicleId
) {
    if (game == 0) {
    ...
    const uintptr_t vehicleSlotAddress =
        game +
        ((uintptr_t)(vehicleId + 0x2A) * 4u) +
        4u;

    return *(uintptr_t*)vehicleSlotAddress;
}

static uintptr_t GetActiveVehicleFromGame(
    uintptr_t game,
    uint16_t* outVehicleId
) {
    if (game == 0) {
        return 0;
    }

    const uint32_t vehicleIndex =
        *(uint32_t*)(game + 0xA8);
...
static void QueueVehiclePosition(
    uint16_t vehicleId,
    float x,
    float y,
    float angle
    ...
    pkt.sequence =
        g_LocalVehicleSequence.fetch_add(1) + 1;
...
// ========================================================================
// CAPTURE LOCAL VEHICLE
// ========================================================================

static void CaptureAndQueueLocalVehicleState(
    uintptr_t game
) {
    if (game == 0 ||
        !g_IsConnected.load()) {
    ...
    const bool positionChanged =
        fabsf(x - g_LastLocalX) >
            POSITION_EPSILON ||
        fabsf(y - g_LastLocalY) >
            POSITION_EPSILON;

    const bool angleChanged =
        fabsf(angle - g_LastLocalAngle) >
        ANGLE_EPSILON;

    // ================================================================
    // ARAÇ HAREKET ETMİYORSA GÖNDERME
    // ================================================================
    ...
    // ================================================================
    // ARAÇ BAŞKA OYUNCUYA AİTSE GERİ GÖNDERME
    // ================================================================

    if (!TryClaimVehicle(vehicleId)) {
        ...
        LOGI(
            "[VEHICLE SEND BLOCKED] vehicle=%u belongs to another player",
            (unsigned)vehicleId
        );
...
// ========================================================================
// APPLY REMOTE VEHICLE
// ========================================================================

static void ApplyRemoteVehicleStates(
    uintptr_t game
) {
    if (game == 0 ||
    ...
    for (uint8_t ownerId = 0;
        ...
        if (g_RemoteVehicles[ownerId].valid &&
            g_RemoteVehicles[ownerId].sequence ==
                state.sequence) {

            g_RemoteVehicles[ownerId]
                .appliedSequence =
                state.sequence;
        }
    }
...
static void HandleSessionWelcome(
    const SessionWelcomePacket& pkt
) {
    if (pkt.ownerId >= MAX_PLAYERS) {

        LOGI(
            "[NETWORK] Invalid assigned player ID: %u",
    ...
    g_LocalPlayerId.store(
        pkt.ownerId
    );

    LOGI(
        "[NETWORK] Assigned local player ID=%u maxPlayers=%u",
        (unsigned)pkt.ownerId,
    ...
    snprintf(
        playerMsg,
        sizeof(playerMsg),
        "Joined multiplayer as Player %u",
...
static void HandleVehiclePositionPacket(
...

    {
        ...
        if (g_VehicleOwner[pkt.vehicleId] ==
            0xFF) {

            g_VehicleOwner[pkt.vehicleId] =
                pkt.ownerId;

            LOGI(
                "[VEHICLE OWNER] vehicle=%u assigned to remote player=%u",
        ...
        if (g_VehicleOwner[pkt.vehicleId] !=
            ...
            LOGI(
                "[VEHICLE REJECTED] vehicle=%u owner=%u expected=%u",
                (unsigned)pkt.vehicleId,
                (unsigned)pkt.ownerId,
                (unsigned)
                    g_VehicleOwner[pkt.vehicleId]
    ...
    RemoteVehicleState& state =
        g_RemoteVehicles[pkt.ownerId];

    if (state.valid &&
        pkt.sequence <= state.sequence) {
        return;
    }
...
static void ResetVehicleSyncState() {
...

    {
        ...
        memset(
            &g_PendingVehiclePacket,
            0,
            sizeof(g_PendingVehiclePacket)
        );
...
void OpenAndroidKeyboard() {
    ...
    if (res == JNI_EDETACHED) {
        ...
        if (g_GlobalJavaVM->AttachCurrentThread(
                &env,
                nullptr
            ) == 0) {
                ...
                if (activityThread != nullptr) {
                    ...
                    if (context != nullptr) {
                        ...
                        if (imm != nullptr) {
                            ...
                            jmethodID toggleSoftInputMethod =
                                env->GetMethodID(
                                    immClass,
                                    "toggleSoftInput",
                                    "(II)V"
...
GLuint LoadTextureFromPNGArray(
    ...
    while (g_IsConnected.load()) {
        ...
        if (bufferedBytes <
            ...
            if (bytesRead > 0) {
                ...
                ShowNativeToast(
                    "Connection Lost (Other player left)!"
                );
        ...
        while (bufferedBytes > 0) {
            ...
            if (packetType ==
                PACKET_SESSION_WELCOME) {

                packetSize =
                    sizeof(SessionWelcomePacket);
                ...
                packetSize =
                    sizeof(VehiclePositionPacket);

            } else {

                LOGI(
                    "[NETWORK] Unknown packet type=%u",
                    (unsigned)packetType
                );
            ...
            if (packetType ==
                ...
                pkt.senderName[
                    sizeof(pkt.senderName) - 1
                ] = '\0';

                pkt.chatData[
                    sizeof(pkt.chatData) - 1
                ] = '\0';
...

        {
            ...
            if (!g_OutgoingChats.empty()) {
                ...
                outMsg =
                    g_OutgoingChats.front();

                g_OutgoingChats.erase(
                    g_OutgoingChats.begin()
                );
        ...
        if (hasOutMsg &&
            ...
            strncpy(
                outPkt.senderName,
                g_Nickname,
                sizeof(outPkt.senderName) - 1
            );

            strncpy(
                outPkt.chatData,
                outMsg.c_str(),
                sizeof(outPkt.chatData) - 1
            );
        ...
        // ============================================================
        // SEND ONLY NEWEST VEHICLE STATE
        // ============================================================
        ...
        if (hasVehiclePkt &&
            ...
            if (!SendAllBytes(
                ...
                ShowNativeToast(
                    "Error: Failed to send vehicle data."
                );
...
void TCPHostThread() {
    ...
    g_ConnectedStatus =
        "Host Started. Waiting for client...";

    ShowNativeToast(
        "Room Created. Waiting for client..."
    );
    ...
    if (g_TcpSocket >= 0 &&
        ...
        if (!SendAllBytes(
            ...
            ShowNativeToast(
                "Error: Failed to initialize multiplayer session."
            );
...
int32_t my_AInputQueue_getEvent(
    void* queue,
    AInputEvent** outEvent
) {
    int32_t result;
...
void my_GameUpdate(
    ...
    // ================================================================
    // IMPORTANT:
    // Remote vehicle sync artık ana GameUpdate içinde.
    // Böylece oyuncu biçerden traktöre geçse bile
    // uzak araç güncellenmeye devam eder.
    // ================================================================

    if (g_IsConnected.load()) {

        ApplyRemoteVehicleStates(
            g_EngineInstance
        );

        CaptureAndQueueLocalVehicleState(
            g_EngineInstance
        );
    }
}
...
void my_GameUpdateStateBase(
    ...
    // Vehicle sync buradan kaldırıldı.
    // Artık my_GameUpdate içinde yapılıyor.
}
...
void ModMain() {
    ...
    LOGI(
        ">>> MULTIPLAYER MOD STARTING <<<"
    );

    ResetVehicleSyncState();

    std::thread(
        StartPONGResponderThread
    ).detach();
